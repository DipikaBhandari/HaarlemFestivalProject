            ...
            <?php if (!empty($section['locations'])): ?>
                <?php foreach ($section['locations'] as $location): ?>
                    <?php if (!empty($location['icon'])): ?>
                        <img id="icon_<?php echo $location['locationId']; ?>" src="<?php echo $location['icon']; ?>" alt=""
                             style="width: 40px; height: 40px; margin-left: 0;">
                    <?php else: ?>
                        <!-- If icon is empty, set background image using CSS -->
                        <div class="icon-placeholder" id="icon_<?php echo $location['locationId']; ?>"
            ...
            <?php if (!empty($section['locations'])): ?>
                <?php foreach ($section['locations'] as $location): ?>
                    <div class="button-col">
                        <div class="buttons">
                            <button id="button" class="timetable-button" onclick="toggleDescription(<?php echo $location['locationId']; ?>)"
        ...
        <div class="locationName" style="margin-left: 0;">
            <?php if (!empty($section['locations'])): ?>
                <?php foreach ($section['locations'] as $location): ?>
                    <div class="name-col">
                        <div class="name">
                            <button id="name_button" class="name-button" onclick="toggleDescription(<?php echo $location['locationId']; ?>)"
    ...
    <div class="card-container" style="margin-left: 60px;">
        <div class="card" style="padding: 20px; border: 1px solid #ccc; border-radius: 5px; width: 800px; height: 700px; background-color: rgba(88, 47, 14, 1); color: white;">
            <div class="cardImage">
                <?php if (!empty($section['locations'])): ?>
                    <?php foreach ($section['locations'] as $index => $location): ?>
                        <img id="image_<?php echo $location['locationId']; ?>" src="<?php echo $location['locationPicture']; ?>"
            ...
            <div class="cardDescription">
                <?php if (!empty($section['locations'])): ?>
                    <?php foreach ($section['locations'] as $index => $location): ?>
                        <p id="description_<?php echo $location['locationId']; ?>"
